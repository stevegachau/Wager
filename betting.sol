...
contract Betting is UsingTellor {
   uint256 public price; 
   uint256 public tellorID;
   uint256 public qualifiedValue;
   uint256 public currentValue;
   bool _didGet;
   uint256 _timestamp;
   uint256 _value;
   uint16 priceBetWinner;
   address payable public owner;
   uint256 public minimumBet;
   uint256 public totalBetsOver;
   uint256 public totalBetsUnder;
   address payable[] public players;
   struct Player {
      uint256 amountBet;
      uint16 priceBetSelected;
    }
// The address of the player and => the user info
   mapping(address => Player) public playerInfo;
   function() external payable {}
   
  constructor(uint _tellorID, address payable _tellorAddress, uint256 _price) UsingTellor(_tellorAddress) public {
      price = _price;
      tellorID = _tellorID;
      owner = msg.sender;
...
function bet(uint8 _priceBetSelected) public payable {
      //The first require is used to check if the player already exist
      require(!checkPlayerExists(msg.sender));
      //The second Over is used to see if the value sended by the player is
      //Higher than the minimum value
      require(msg.value >= minimumBet);
//We set the player informations : amount of the bet and selected priceBet
      playerInfo[msg.sender].amountBet = msg.value;
      playerInfo[msg.sender].priceBetSelected = _priceBetSelected;
//then we add the address of the player to the players array
      players.push(msg.sender);
//at the end, we increment the stakes of the priceBet selected with the player bet
      if ( _priceBetSelected == 1){
          totalBetsOver += msg.value;
      }
      else{
          totalBetsUnder += msg.value;
      }
    }
    ...
    function distributePrizes() public {
     ...
     (_didGet,currentValue,_timestamp) = getCurrentValue(tellorID);
      ...
      if (currentValue > price) {priceBetWinner = 1;}
      else {priceBetWinner = 2;}
      
      
      
//We loop through the player array to check who selected the winner priceBet
      for(uint256 i = 0; i < players.length; i++){
         playerAddress = players[i];
//If the player selected the winner priceBet
         //We add his address to the winners array
         if(playerInfo[playerAddress].priceBetSelected == priceBetWinner){
            winners[count] = playerAddress;
            count++;
         }
      }
//We define which bet sum is the Loser Over and which Over is the winner
      if ( priceBetWinner == 1){
         LoserBet = totalBetsUnder;
         WinnerBet = totalBetsOver;
      }
      else{
          LoserBet = totalBetsOver;
          WinnerBet = totalBetsUnder;
      }
//We loop through the array of winners, to give ethers to the winners
      for(uint256 j = 0; j < count; j++){
         ...
         if(winners[j] != address(0))
            ...
            //Transfer the mOvery to the user
...
function AmountOver() public view returns(uint256){
       return totalBetsOver;
    }
function AmountUnder() public view returns(uint256){
       return totalBetsUnder;
    }
